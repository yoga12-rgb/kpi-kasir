-- ============================================================
-- 0032_race_safe_setup.sql
--
-- M2.3: setup admin pertama memakai reservation claim agar request
-- paralel tidak dapat membuat dua admin Auth.
-- ============================================================

alter table public.app_setup
  add column setup_claim_id uuid,
  add column setup_claimed_at timestamptz,
  add column setup_attempt_count integer not null default 0,
  add column setup_attempt_window_at timestamptz;
...
create or replace function public.release_setup(p_claim_id uuid)
returns boolean
language plpgsql
security definer
set search_path = public
...
begin
  ...
  update public.app_setup
  set setup_claim_id = null,
      setup_claimed_at = null
  where setup_claim_id = p_claim_id;
...
revoke all on function public.release_setup(uuid)
  from public, anon, authenticated;
grant execute on function public.release_setup(uuid) to service_role;
