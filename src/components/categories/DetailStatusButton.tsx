...
interface DetailStatusButtonProps {
  categoryId: string;
  detailId: string;
  detailName: string;
  isActive: boolean;
...
export function DetailStatusButton({
  ...
  async function changeStatus() {
    ...
    try {
      const response = await fetch(`/api/categories/${categoryId}/details/${detailId}/status`, {
        method: 'PATCH',
      ...
      if (!response.ok) {
        setError(getErrorMessage(data?.error, `Gagal ${actionLabel.toLowerCase()} detail`));
        return;
      }
  ...
  return (
    <>
      <button
        ...
        aria-label={`${actionLabel} detail ${detailName}`}
        title={actionLabel}
      >
      ...
      >
        <div className="space-y-3 text-sm text-surface-600">
          <p>
            Detail <strong className="font-semibold text-surface-900">{detailName}</strong> akan
            menjadi {isActive ? 'arsip' : 'aktif'} untuk konfigurasi periode berikutnya.
          </p>
          <p>Assessment dan snapshot periode berjalan tetap tersimpan.</p>
        </div>
        ...
        <div className="mt-4">
          <Textarea
            ...
            placeholder="Contoh: Detail diganti dengan indikator baru"
            minLength={3}
            maxLength={500}
            required
            disabled={loading}
