...
| S4 | P2 | **CSP mengizinkan `'unsafe-inline'` untuk script** — melemahkan mitigasi XSS; inline script diizinkan. Dibutuhkan oleh Next.js App Router untuk flight/hydration script; menghapusnya memutus render klien (skeleton tidak ter-resolve). Dipertahankan sampai mekanisme nonce diterapkan. | `next.config.mjs` `script-src 'self' 'unsafe-inline'...`. |
